...
public class OrderIService {
	...
	private static final String ORDER_SERVICE = "BAITU-PRODUCER-ORDER";
	private static final String ORDER_SERVICE_IP="127.0.0.4";
	...
	public BtOrder method1() {
		BtOrder order = 
				restTemplate.getForObject("http://"+ORDER_SERVICE_IP+":"+ORDER_SERVICE_PORT+"/orders/info?{1}", BtOrder.class, "order-1");
	...
	public BtOrder method2() {
		ResponseEntity<BtOrder> responseEntity = 
				restTemplate.getForEntity("http://"+ORDER_SERVICE+"/orders/info?{1}", BtOrder.class,"order-2");
	...
	public BtOrder method3() {
		...
		String url = String.format("http://%s:%d/orders/info?{1}",serviceInstance.getHost(),serviceInstance.getPort());
